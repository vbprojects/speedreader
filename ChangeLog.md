...
- **Demand-driven Jetstream**: Every accepted Bluesky post is now persisted and rendered as soon as its filtering and enrichment finish. The socket uses bounded post windows instead of 400-word chunks: by default it inserts a wake trigger with five posts left in a 25-post window, disconnects at the window boundary, and resumes from its retained cursor when the reader reaches the trigger. Older cached live streams perform one bootstrap pull to acquire their first wake boundary.
