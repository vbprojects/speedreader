...
- A deployment must provide a first-party gateway when a catalog does not permit browser CORS. Gateways remain subject to the same origin, credential, size, and provenance rules. A plugin selects the allowlisted `gutenberg`, `catalog`, or `twine` route; the host owns the configured gateway base URL and the Worker independently validates every target. The `twine` route accepts only an IFDB TUID plus a server-resolved offer identifier, never an arbitrary external target.
...
- `org.ifdb.twine` uses [IFDB's official JSON search API](https://ifdb.org/api/search) with its required Twine system constraint. Opening the source uses a small bundled featured shelf and makes no request; explicit searches make one throttled, session-cached API request. Opening a result lazily makes one cached [viewgame](https://ifdb.org/api/viewgame) request to discover downloadable and playable releases. Supported IF Archive HTML/ZIP releases are acquired through the constrained `twine` Worker route. If IFDB has no supported release, the Worker searches IF Archive by exact TUID. The canonical IFDB source-page option remains available for external hosts and missing or ambiguous packages.

Generic ZIP ingestion is limited to published SugarCube archives. It applies the same entry-count, expanded-size, per-entry-size, and compression-ratio ceilings as EPUB preflight, rejects unsafe paths, prefers a root `index.html`, and refuses ambiguous archives. The selected HTML and a bounded set of sibling assets are persisted together; a future runtime host can expose those assets through an isolated virtual origin.
